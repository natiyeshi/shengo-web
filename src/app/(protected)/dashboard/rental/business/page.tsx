        ...
        <Tabs defaultValue={defaultTabValue}>
          ...
          {Object.keys( RENTAL_RESIDENCE_TABS_MAP).map((tabKey) => (
            <TabsContent key={tabKey} value={tabKey}>
              { RENTAL_RESIDENCE_TABS_MAP[tabKey] ===
                 RENTAL_RESIDENCE_TABS_MAP.lessor && (
                <CustomerIdentityRegisteration type="Lessor" />
              )}
              { RENTAL_RESIDENCE_TABS_MAP[tabKey] ===
                 RENTAL_RESIDENCE_TABS_MAP.renter && (
                <CustomerIdentityRegisteration type="Renter" />
              )}
              { RENTAL_RESIDENCE_TABS_MAP[tabKey] ===
                 RENTAL_RESIDENCE_TABS_MAP.business && <VehicleRegistration />}
              { RENTAL_RESIDENCE_TABS_MAP[tabKey] ===
                 RENTAL_RESIDENCE_TABS_MAP.withness && (
                <CustomerIdentityRegisteration type="Witness" />
              )}
              { RENTAL_RESIDENCE_TABS_MAP[tabKey] ===
                 RENTAL_RESIDENCE_TABS_MAP.service && <ServiceRequest />}
            </TabsContent>
          ))}
        </Tabs>
      </section>
