...
const Page = () => {
  const defaultTabValue = Object.keys(RENTAL_RESIDENCE_TABS_MAP)[0];
  return (
      ...
      <section className="">
        <Tabs defaultValue={defaultTabValue}>
          <TabsList className="mb-7 rounded-md py-1">
            {Object.keys(RENTAL_RESIDENCE_TABS_MAP).map((tabKey) => (
              <TabsTrigger className="rounded-md" key={tabKey} value={tabKey}>
                {RENTAL_RESIDENCE_TABS_MAP[tabKey]}
              </TabsTrigger>
            ))}
          </TabsList>
          ...
          {Object.keys(RENTAL_RESIDENCE_TABS_MAP).map((tabKey) => (
            <TabsContent key={tabKey} value={tabKey}>
              {RENTAL_RESIDENCE_TABS_MAP[tabKey] === RENTAL_RESIDENCE_TABS_MAP.lessor && (
                <CustomerIdentityRegisteration type="Lessor" />
              )}
              {RENTAL_RESIDENCE_TABS_MAP[tabKey] === RENTAL_RESIDENCE_TABS_MAP.renter && (
                <CustomerIdentityRegisteration type="Renter" />
              )}
              {RENTAL_RESIDENCE_TABS_MAP[tabKey] === RENTAL_RESIDENCE_TABS_MAP.residence && (
                <VehicleRegistration />
              )}
              {RENTAL_RESIDENCE_TABS_MAP[tabKey] === RENTAL_RESIDENCE_TABS_MAP.withness && (
                <CustomerIdentityRegisteration type="Witness" />
              )}
              {RENTAL_RESIDENCE_TABS_MAP[tabKey] === RENTAL_RESIDENCE_TABS_MAP.service && (
                <ServiceRequest />
              )}
            </TabsContent>
          ))}
